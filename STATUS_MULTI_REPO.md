...
- **Game**: maratona 07-06/07 landed su main -- **LOS default ON** (#3226 + step prod default), **big-maps arc: 3 encounter grid_sized MERGED** (16x12 dorsale #3229 / 20x12 canyon #3230 / 18x10 colata basaltica #3237; ADR-2026-07-03 board_scale ACCEPTED, wiring #3199), **seed-fix #3232** (hash string seeds -> full-loop finalmente seedabile; batch storici = unseeded), D4 A/B roster-scaling negative (#3231, flag OFF), zone-defense spec #3238, xp-budget geometry calibration #3239, seed census #3240. **App-token automation #3242 MERGED** (delivery PR bot via GitHub App token, fix gate action_required). **0 PR open su TUTTA la family** (Game/GGv2/Game-DB, verificato 07-10).
...
- **Godot-v2**: campagna Jules a 149/285 (52%), pool filler residuo 5 (~2 trio all'hard-stop). Riconcile overlay PRD post fleet-verify -> **draft-PR GGv2 #595** (merge = Eduardo).
- **vault**: riconcile SoT (LOS default ON sez 14.4/14.5 + board_scale/3-encounter sez 14.1/15.1 + 15-LEVEL_DESIGN) -> **draft-PR vault #269** (merge = Eduardo, mai diretto).
