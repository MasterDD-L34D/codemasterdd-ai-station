...
- [ ] **C1** -- Ryzen `Desktop\Game` sandbox cleanup. Stato post-ADR-0027 empirical: clone STALE 107 commits behind origin/main + working tree dirty (apps/backend/* deletions). Eduardo-direct decision: (a) reset --hard origin/main + commit any explorative changes via branch nuovo, (b) deletion full clone + re-clone fresh se serve, (c) preserve as-is (cosmetic clutter, no operational impact). Low priority — NO operational impact.
- [ ] **C2** -- Ryzen `Desktop\repos\codemasterdd-ai-station` Codex branch cleanup. Stato: branch `codex/structural-reset` HEAD `4b7c84a` 6 ahead origin/main (NOT main). Eduardo-direct decision: (a) merge utili changes su main + push (improbable, Codex recovery-related), (b) delete branch + re-clone fresh, (c) preserve. Low priority — sandbox abbandonato, no active workflow.
- [ ] **Q3-update** -- Add 5 active Ryzen-only repos a STATUS_MULTI_REPO.md (1-line entry each): claude-supermemory-local, compass-marketplace, Game-Database, Master-DD-Pathfinder-GPT, torneo-cremesi-site. 4 dormant left silent-driver autonomous (Gpt, Item-generator, LeaD, pathfinder-1e-homebrew). Trigger ADR addendum se uno dormant si attiva con scope strategic. Low priority — informational only.
