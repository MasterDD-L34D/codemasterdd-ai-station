...
| 0024 | Vue3 archive + Godot v2 canonical timeline | **Rejected (reconciled 2026-06-08)** | 2026-05-09 | **RECONCILE 2026-06-08**: archive-codebase 09-30 RITIRATO -- cutover gia' eseguito (Game ADR-2026-05-05: Phase A 05-07 + Phase B 05-14, backend preserved by design). Modello canonico = Game backend/sim/canon (NON archiviato) + Godot frontend; solo frontend web `apps/play` archiviato 14/05. Review 08-01 + trigger 60gg/parity = MOOT. (Orig 2026-05-09: soft-deadline archive Vue3 09-30 + AA01 Ondata 1+2 H11.) |
...
| 0027 | Cross-PC clone architecture Lenovo+Ryzen | **Accepted (early 2026-05-13)** | 2026-05-13 | Fleet 4 PC LAN: Lenovo `.10` agentic hub + Ryzen `.11` inference secondary 4070S 12GB. DHCP reservation permanent <home-router>. SSH key-based auth. |
...
| 0037 | Merge-autonomy model (standing vs Eduardo-explicit) | **Accepted 2026-06-03** | 2026-06-03 (P -> A stesso giorno, Eduardo "fai la ratifica") | Quasi nulla e' standing: merge repo-esterni = Eduardo-explicit; file governance-doctrine = Eduardo-only-merge in QUALSIASI repo; auto-merge default-OFF, earn-gated. Estende ADR-0036. Survived SDMG harsh-reviewer falsification. |
| 0038 | Doctrine carve-out completion (amends 0037 dec.2) | **Accepted 2026-06-11** | 2026-06-03 (P) -> 2026-06-11 (A) | Emenda SOLO ADR-0037 decision 2; il resto di 0037 resta valido. SDMG falsification 2026-06-03 SURVIVE-WITH-CHANGES. Accepted con 4 amendment testuali + sync `actor-activation-criteria.md` sec 7 same-PR (dossier `docs/research/adr-0038-0039-ratify-dossier-2026-06.md`). |
| 0039 | R1 open-PR reconcile rung | **Accepted 2026-06-11** | 2026-06-03 (P) -> 2026-06-11 (A) | Implementa il rung che ADR-0037 dec.4 nominava come unblocker (allora non-built). 3a SDMG falsification BUILT code 2026-06-03 SURVIVE-WITH-CHANGES. Accepted con amendment P1 clock-free rescope (STATUS-leg cycles NON contano per R2 fino a fix clock-leak eng-graph; vault-leg conta) + 3 annotazioni R2 (b)-(d) (dossier ratifica). |
| 0040 | Doctrine PR triage label (hint-only) | **Proposed 2026-06-17** (ratified on Eduardo merge) | 2026-06-17 | Doctrine files (`.claude/**`, `docs/adr/**`, `docs/governance/**`, root rules) = Eduardo-only-merge. Deterministic regex label = HINT-only; red-team REFUTED un auto-merge fast-lane (regex non distingue capability-expanding da neutral). Impl: `scripts/governance/doctrine_triage.py` + test. Estende ADR-0037. PR #367. |
| 0041 | Sovereign local-SD asset generation (ornate-decorative only) | **Proposed** | 2026-06-18 | Supervised spike su Ryzen (RTX 4070S): local Stable-Diffusion come spoke content-gen delegato. ADOTTATO solo per asset ORNATE-DECORATIVE single-object (medaglioni 6/8 pass falsification bar); BOUNDED-OUT per flat tactical markers + UI panel funzionali (falliti, restano manuali). Nulla promosso: output staging-only, approvazione Eduardo per-asset. Estende ADR-0036. |
| 0042 | evo-swarm entity-grounding verification arc (3 lever) | **Proposed** | 2026-06-20 | L1 entity-grounding gate + L3 constrained canonical_refs ADOPTED+landed (evo-swarm #124-129); L2 redundancy-checker SDMG-REJECTED -> archived learning-record (#400); OD-007 biome-SoT loader bug fixed (#128). Lezioni: SDMG ha preso 2/3 lever errati pre-ship; recon=ipotesi; format-vale-solo-col-grammar-pattern (A/B 100% vs 71% well-formed ref). |
| 0043 | Code-graph tooling adoption (CodeGraph + graphify) | **Accepted 2026-07-02** | 2026-07-02 | Game repo family: adottati ENTRAMBI (ruoli distinti, bake-off complementare) SENZA hook (Windows flash-safety); claude-mem resta OFF (shadow-dup + flash); zero footprint tracked (`.git/info/exclude`). GDScript gap: ciechi su Godot `.gd` (follow-up tree-sitter-gdscript). Evidenze: QUALITY.md. |
| 0044 | Personal Agentic OS: composizione, non costruzione | **Proposed 2026-07-12** | 2026-07-12 | L'"Agentic OS" esiste gia' ~90% come federazione (ORCHESTRATION.md + routing + subagent + memoria + hook + AA01 + cron). Build = mappa root `AGENTIC_OS.md` (7 layer -> authority) + guard-test link-existence + roadmap gap G1-G3 Eduardo-gated. Reject framework esterni (shadow-dup) + runtime nuovo (SDMG/OD-009). Ricerca: last30days + deep-research (13 claim 3-0). |
...
- **ADR-0024** -- Vue3 Game archive + Godot v2 canonical timeline 2026-09-30. Risoluzione edge case HIGH "Game pivot Vue3->Godot definitivo pre-settembre". Soft-deadline 4 mesi + review trimestrale + trigger 60gg silenzio. AA01 attivazione esplicita Ondata 1+2 (Eduardo direct H11). **[RECONCILED -> Rejected 2026-06-08: archive-codebase RITIRATO, cutover gia' eseguito; vedi tabella riga 0024 + ADR-0024 addendum.]**
...
- **Azioni derivate**: ADR-0034 draft + harsh-reviewer dispatch. **Esito 2026-05-18**: arbitro esterno **REJECT-redux Option A conf 84%** (R3=msg-A5-fallito-riemesso, R1-valida-solo-meta-sicura, R4-kill-switch=gate-gia-fallito, R2-decider=euristico-69%FP-vietato-SDMG, accretion L-033). SDMG pre-commit onorato (Option A NON difesa). **Option D adottata** (draft-batch + auto-archive-only-non-generativo + Eduardo 1 batch-approve/ciclo = fuori loop, intento mandato soddisfatto, zero autonomia corrective-send su agent incontrollabile).
- **Update 2026-06-01 (git-truth)**: ADR-0034 **ACCEPTED Option D** confermato da Eduardo "si confermo" (commit 814a41f). Catena governance Jules costruita sopra, tutta Accepted: ADR-0034 (Accepted Option D) -> **ADR-0035** (Jules-from-CLI proactive dispatch async-remote-agent, Accepted 2026-05-29, clean-rate 5/5) -> **ADR-0036** (Unified Orchestration Doctrine multi-LLM + Jules + Opus 4.8, Accepted spine 2026-06-01, auto-merge rung deferred a earn-path). La nota precedente "Pending Eduardo accept Option D / NO azione Jules" e' SUPERATA: Option D accettata e operativa.
...
- **Process-fix partial** (git-truth 2026-06-01): `STATUS-CHECK:` machine-greppable applicato a 6 ADR non-final (0016/0021/0022/0024/0029/0031, commit 3d0957c) + runbook con schema/grep-one-liner/escalation. Cron settimanale anti-STALE-STATUS ancora TODO (la weekly Task Scheduler e' documentata nel runbook ma non ancora schedulata).
