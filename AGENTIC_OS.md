...
## Principi dalla ricerca (2026-07-12, fonti in [report](docs/research/personal-agentic-os-2026-07-12.md))

**Verificati** (voto adversariale 3-0, fonti primarie -- docs ufficiali code.claude.com + arXiv 2604.14228):
- I subagent tornano SOLO summary al parent: composizione = file condivisi, non contesto condiviso.
- Memoria: indice conciso + topic file on-demand (MEMORY.md: primi 200 righe / 25KB caricati a inizio sessione).
- CLAUDE.md = advisory (nessuna garanzia di compliance), enforcement duro = SOLO hook (PreToolUse).

**Direzionali / segnali community** (NON verificati 3-0, ma coerenti con la dottrina gia' in essere):
- "Skill teaches the how, Hook enforces the rule, Subagent isolates the work" (segnale community, ofox.ai).
- Multi-agente costa ~15x token di una chat (direzionale, blog Anthropic): gia' riflesso nel cost ladder ORCHESTRATION.md sec 3.
- CLAUDE.md <200 righe + procedure -> skill (guidance): gia' applicato con la policy reorg 2026-06-03.
- "Se lo rispiegheresti a un neoassunto, va in memoria persistente" (euristica di persistenza).
