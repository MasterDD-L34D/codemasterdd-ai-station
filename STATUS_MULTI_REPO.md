...
| game-governance-drift | warning | 0 errors, 20 warnings (20 total) | 2026-06-23T21:20:30+00:00 | https://raw.githubusercontent.com/MasterDD-L34D/Game/main/reports/docs/governance_drift_report.json |
...
| jules-digest | ok | jules digest 2026-06-23: 0 awaiting | 2026-06-23 | https://api.github.com/repos/MasterDD-L34D/codemasterdd-ai-station/contents/docs/jules-batch/2026-06-23-digest.md?ref=main |
| vault-coherence | warning | coherence report 2026-06-28: BLOCK 0 WARN 9 | 2026-06-28 | https://api.github.com/repos/MasterDD-L34D/vault/contents/Extras/lint-reports/coherence-2026-06-28.md?ref=main |
...
| vault-gap | warning | gap report 2026-06-25: 3/5 summary metrics nonzero | 2026-06-25 | https://api.github.com/repos/MasterDD-L34D/vault/contents/Extras/lint-reports/gap-2026-06-25.md?ref=main |
...
**Ultimo refresh**: **2026-06-30** (full 7-repo, repo-health-auditor gh-api) + **delta game-family 2026-07-03** (fleet-verify da Lenovo) + **delta 2026-07-10** (post fleet-verify 07-09 + ratifica flag-set owner, da Ryzen) + **delta 2026-07-13** (fleet-verify game-family da Lenovo, 0 RED). NB: la **Snapshot 1-riga** + sezioni per-repo narrative sotto restano contesto datato (last-verified per-sezione); il layer fresco = la sezione "Audit delta 2026-07-13" (in fondo alla pila) qui sotto (le delta accumulano newest-last, lo Snapshot non si ri-hardcoda -- daily-ship).
...
### Audit delta 2026-07-03 (fleet-verify game-family -- 2 run concorrenti consolidati: Workflow 5-probe + run Lenovo 3-probe/SSH)

> Scope game-family (Game/Godot-v2/Game-DB), NON 7-repo (evo-swarm/vault/Synesthesia: layer 06-30 resta fresco, unica novita' vault sotto). HEAD non ri-hardcodati (git-truth via gh, regola :23). Due sessioni hanno fatto fleet-verify oggi -> questo e' il merge ground-truthed: gli stati #3195/#585 riflettono il ground-truth PIU' TARDIVO (= merged). gh-auth VIVO su entrambe le macchine.

- **Game**: daily-ship attivo (`83cf6e7d` -> `6703f782` -> oltre in-sessione). **#3195 MERGED** questo giro (char-test generate_open_decisions pure helpers; rebase-merge trailer ADR-0011 preservati; gate strict rispettato via `update-branch` x2, no `--admin`) -- supera il "1 PR open do-not-merge" del primo run. Residui = PR daily-automation (#3198 fase-2c grid-wiring, #3196 daily-tracker-refresh). Main CI GREEN (CI + Docs Governance + SoT Drift Sentinel + Skiv Monitor success; Daily-PR-Summary fail = job reporting non-gating). **MAP-Elites v2 arc CLOSED** (#3183 merged 07-02, cfr BACKLOG M15). SoT-drift: 0 candidate aperti (sentinel 12/12). Design-direction = AMBER/"in costruzione": ultimi ~10 commit balance-heavy (~60%: xp_budget geometry + activation-pressure Shape 1/2 + grid-ratify L-069), finestra 20-commit piu' governance-heavy (doc-pins 10/20); balance-calibration (MAP-Elites hc06 `enemy_damage` lever, SPRT truncated-eviction) = legit-core; rischio mono-pilastro fairness (pilastri 2-5 fermi); docs open-decisions SPEC-E/SPEC-G TODO 2-7wk stale da riconfermare. **Clone Lenovo IGIENIZZATO** questo giro: era detached 50-behind + 698 dirty (merged-leftover, work Game vive su Ryzen) -> `checkout -f main` + `pull --ff-only`, 8 scratch rimossi, 2 doc tenuti; ora main current, clean.
- **Game-Godot-v2**: **#585 CombatCamera zoom-to-fit MERGED** (origin/main `89b6b81f`), 0 PR open, GUT test verde. (Il primo run l'aveva colto OPEN + UNSTABLE gdformat-lint mid-flight; poi merged.) Ryzen ha branch attivo `feat/combat-camera-controls` (follow-up #585, monitor-only, NON pushare -- collision). Clone Lenovo su branch merged-leftover portrait-loader (#556 merged, innocuo).
- **Game-Database**: **evo-import-sync FIX landed** -- PR #233 MERGED 07-03 (unblock bot-403 push + `_game` gitlink gitignore); workflow "Evo Import Sync" era failing 8+/8 scheduled fino ~09:36, verificato GREEN via manual dispatch (run 28656614132, job `sync` reale non no-op); origin/main `6aa7687`. **Prisma schema drift = 0** (Trait schema.prisma:133-172 + Species/provenance 1:1 su migration + import-taxonomy.js). 0 PR open. Clone Lenovo era behind 5 -> pullato current (package-lock fsevents-drift benigno scartato).
- **vault <-> Game SoT d20**: **0 RED**. Costanti combat esatte vs Game@`83cf6e7d`: action_speed, status_penalty, ap_max=2 + eccezione tutorial-01 (ap=3). 1 AMBER path-citation: `11-REGOLE_D20_TV.md:61` cita `services/sessionRoundBridge.js`, reale `routes/` -> **draft-PR [vault #266](https://github.com/MasterDD-L34D/vault/pull/266)** (cosmetico, Eduardo-merge; mai auto-merge vault).
...
### Audit delta 2026-07-13 (fleet-verify game-family, Lenovo, Workflow 5-probe + 3-vote verify)

> Scope game-family (Game/GGv2/Game-DB) + SoT vault + governance-prose. HEAD non ri-hardcodati (git-truth via gh). gh-auth VIVO. **0 RED confermati.**

- **Game**: origin/main `a639d5f5b` (#3276 fa-polish-badlands-plan). CI main GREEN (Daily tracker + Skiv Monitor). 1 PR open = #3275 weekly-drift-audit DRAFT (routine). Design-direction: arco studio-track fase 3 (F-A..F-D slice-first) -- ultimi ~15 commit dal 07-11 = **SOLO docs/plans/roadmap/assets** (audio foundation #3273, vfx combat #3274, polish+badlands #3276, GDD refresh, asset manifest); ZERO data/core/runtime touch. Balance gate confermato composite 0.70*WR+0.30*KD, pe_ratio dormiente (canonical-suite.yaml). AMBER minore: knob hc07 `enemy_damage_multiplier_override=2.5` = tetto del search space (zero headroom; WR forensic N=100 = 42% mid-band, non-bloccante).
- **Game-Godot-v2**: origin/main `3e6039679` (#602 fa-polish-badlands). 0 PR open, workflow "test" GREEN ultimi 3 push (audio #599, vfx #601, polish #602). Studio-track fase 4 f-a 3/3 eseguito.
- **Game-Database**: origin/main `13a38ab8c` (#238 honest evo-import smoke). 0 PR open, Evo Import Smoke 3/3 schedulati GREEN oggi. **Prisma drift = 0** (schema.prisma vs 13 migration, corrispondenza completa). Pipeline evo-import coerente end-to-end. AMBER: nessun CI gate automatico `prisma migrate diff` (correttezza su disciplina manuale documentata).
- **vault <-> Game SoT d20**: **GREEN**. Nessun runtime change su Game main dal 07-10 -> nessun drift costanti possibile dall'arco 07-12/13 (planning-only). Unico open = **vault PR #266** DRAFT (path-citation cosmetica `sessionRoundBridge` services->routes, dal 07-03; hand-merge Eduardo, mai auto-merge vault). Vault fa coherence backstop 07-12/13 (PASS-3).
- **Governance-prose**: STATUS/GOALS freshest layer = 07-10 delta; arco 07-12/13 (OS Console #552-#564 + fa studio-track) non ancora nel file -> questo delta lo aggiunge. Snapshot 1-riga + governor-sync block restano datati BY DESIGN (disclaimer righe 21/23). AMBER: blocco GOVERNOR-SYNC (segnali 05-27..06-28 stale) -> #564 (revive R0 signal pane, freshness + daily ingest, landed 07-13) deve rinfrescarlo al prossimo ingest -- verificare.
- **Igiene cloni Lenovo (delta macchina-specifico, read-only, NON toccato)**: Game clone main BEHIND-99 + 2 untracked scratch (`docs/ops/backend-components-inventory.md` = doc locale non landato AMBER; `logs/monitor_map_elites.py` = scratch probabilmente superseded, tooling canonico gia' su main sotto tools/py). GGv2 clone su branch `feat/creature-portrait-loader` (PR #556 MERGED 06-29 squash -> 3 commit locali = merged-leftover, cleanup = checkout main quando no sessione attiva; NON reset ora, collision-safe). Game-DB clone in sync (package-lock local-only = fsevents-drift benigno noto).
- **Backend prod EvoTacticsBackend NON toccato.** Deliverable: Artifact health-report + questo delta. Nessun draft-PR di fix (i 2 untracked = lane Ryzen no-push da Lenovo; governor-sync = auto-refresh via #564).
...
| **Dafne swarm (evo-swarm)** | **Verification arc CLOSED 2026-06-20**: 3-lever gate done (L1 entity-grounding #124 ratified #127 / L2 REJECTED SDMG / L3 constrained canonical_refs SHIPPED #129 live-loop). **Decisione 013 #130: swarm-as-production-accelerator FALSIFIED** (12 artifact -> 11 rejected 91.7% hallucination via L1/L3 gate; kill-criterion met; qwen3-coder:30b unfit per design-synthesis) -> swarm-generator ARCHIVED, verify-swarm-claims promosso linter standalone. Repo pivot OBSERVABILITY: dashboard fase 2-8 (#97-#102). Runtime PARKED. **GitHub remote ARCHIVED+PRIVATE 2026-06-22** (isArchived=true, push 403). Swarm->Game digest pipeline CHIUSA (8+wk 0-cicli, picco 04-27 432-cicli -> 0/0); 13 report post-mortem archiviati `docs/archive/evo-swarm-digest-archive/`. 0 PR open. | Nessuna decisione pendente (013 NO-GO + remote archived); dashboard observability = Eduardo/AI-driven | -- | Nessuno |
