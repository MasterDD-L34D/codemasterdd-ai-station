...
| **playtest#2 automation (OD-044)** | ✅ Envelope A+C SHIPPED | Game **PR #2282+#2283 merged**: bridge sim→analyzer + nightly-cron step + weekly `playtest-2-weekly.yml` (450-run, gate ≥30). **6/7 dimensioni real** (P3-promo/P4-4layer/P6-fairness/OD-024/perf — 2 bug backend pre-esistenti fixati). OD-026 = solo residuo, deferred onesto (pulse vive Godot-client, sim guida backend Express — non gap-tooling). PT2-A=🟢-hard → P3/P4/P6/OD-024/perf promovibili 🟢 da run≥30. Validato locale end-to-end (§7). Envelope B (regressione/auto-flow) = follow-up opzionale. Vault OD-044 |
