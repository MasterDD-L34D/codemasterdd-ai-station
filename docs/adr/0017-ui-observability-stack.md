...
- **Status**: **SUPERSEDED-by-ADR-0030 (Hybrid A1)** 2026-05-28 sera. Originalmente Accepted 2026-05-07 (5/5 criteri ratification PASS pre-pivot). Post pivot ADR-0030 Accepted 2026-05-18, routing primario = Pro+Meridian (via OpenCode bridge) + Gemini-CLI direct + wrapper aider-* direct -- NON via LiteLLM proxy. Stack codice rimosso 2026-05-28 sera (`infra/` + `apps/dogfood-ui/` deleted) via OD-009 decommission decision (online sources convergent: solo-dev < $100/mo = SDK/no-proxy; Langfuse self-host = task admin senza valore per il profilo). Reversibile via `git revert` o `git checkout <pre-commit> -- infra/ apps/dogfood-ui/` se future need emerge. **scripts/quality-bench/** retained standalone (no LiteLLM dependency). Closure doc: OPEN_DECISIONS OD-009 + `docs/research/2026-05-28-codemasterdd-first-principles-audit.md`.
