...
| **playtest#2 automation (OD-044)** | ✅ COMPLETO (A+B+C+OD-026) — ceiling 6.5/7 | Game **PR #2282+#2283+#2284+#2285 merged**. A+C: bridge sim→analyzer + nightly-cron + weekly `playtest-2-weekly.yml` (450-run, gate≥30). **OD-026 (PR #2285)**: `senseReveal.js` seam wired → `skiv_pulse_fired` REAL (7 pulse-event verificati, target_biome_id da stato sessione) → **6.5/7**; `biome_focus_changed` deferred onesto (camera/UI **solo client Godot**, nessun seam backend — ceiling architetturale, non gap-fixabile lato Game). 3 bug backend pre-esistenti fixati lungo l'arco (promotionEngine job_id→job, promotions.yaml gate irraggiungibile, default_parts droppato). Cron-real proxy 36-run validato (bootstrap-window verde). **B (PR #2284)**: `check-pillar-baselines.js` rolling regression baselines su 7 pillar dims (P3-promo-rate/P4-conviction-stdev/P4-4layer/P6-pressure-spread/P6-rewind%/OD-024-firing%/perf-p95) — coverage drift-detection 0→7 (era WR-only); `by_profile_scenario` cross-tab (chiude blind-spot aggregato); bootstrap window PT2-D (primi 5 run silenziosi); auto-issue `playtest-2,automated` su regressione/verdict-change; baseline+report via PR non auto-commit a main (PT2-C). B4 board-sync = **manual-handoff onesto**: workflow GITHUB_TOKEN Game-scoped non può PR cross-repo → su verdict-change scrive `playtest-2-board-delta-<date>.md` nell'artifact, questa riga sincronizzata via PR manuale/cron (questo PR = esempio one-time stato corrente). Verdict pillar corrente: **bootstrap** (baseline samples 0; verdict armato da run≥5). PT2-A=🟢-hard → P3/P4/P6/OD-024/perf promovibili 🟢 da run≥30. Vault OD-044 |

<!-- AUTO-SYNC:playtest2 BEGIN -->
**Auto-sync (OD-044, last refresh 2026-05-17 UTC)** — source: Game `tools/sim/pillar-baseline.json` @ `main` (raw-fetch, OD-042-A pattern). Pillar verdict: **bootstrap** · baseline samples: **0** · baseline updated_at: `n/a`. Latest `ai-sim-nightly` run: **success** (2026-05-16) → [run log](https://github.com/MasterDD-L34D/Game/actions/runs/25953929005). _Auto-refreshed signal only; human prose below is authoritative for context._
