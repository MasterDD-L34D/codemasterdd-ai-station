...
**Ultimo refresh**: **2026-05-27** (deep cross-repo audit: 8 repo, git-truth, PR scan, service check — repo-health-auditor). _Nota: 26 commit remote integrati via rebase 2026-05-27 (dashboard unify + genetic-model Fase-1/2/3); dettaglio genetic in `docs/sessions/2026-05-26-genetic-model-design-closure.md` + JOURNAL._
...
| **Game (Evo-Tactics Vue3)** | HEAD `196a63a4` local **stale: 71 behind** origin/main `637b3dc0` (audit 2026-05-27; FF-pull bloccato da skip-worktree `.husky/pre-commit`, vedi KNOWLEDGE_MAP §6). Clean working tree. **DRAFT PR #2385** open (weekly drift audit 2026-05-25). Recent: genetic model Fase-1 closure + Fase-2 DONE (#2399/#2400) + Fase-3 epigenome RATIFIED #2401 (dettaglio in `docs/sessions/2026-05-26-genetic-model-design-closure.md`). Daily-ship active. | Triage DRAFT PR #2385 + sync local (KNOWLEDGE_MAP §6) | -- | Local 71 behind |
...
| **vault-shared (Vault Knowledge Mgmt)** | Local `C:/dev/vault` HEAD `9880f4da0` (2026-05-25, obsidian-wiki runbook). Dirty: 1 modified (.gitignore) + 2 untracked (SearchReplaceNoExactMatch, aider_test.txt). **4 DRAFT PRs open** (#180/181/190/201 coherence-backstop daily series 24-27/5 -- triage needed, likely Eduardo-merge-gate). Recent: eng-graph query improvements. Privacy: sovereign-only. | Triage 4 open DRAFT PRs (Eduardo-merge-gate) | -- | Nessuno |
| **Game-Database** (Ryzen `C:\dev\Game-Database`, no Lenovo clone) | HEAD `13079e2` (2026-05-25). 0 PR open. **Phase C-Game versioning active**: versioned trait reads API (#163) + version-management UI (#164) + RFC design (#166). Jules/Claude active. | Monitor (Eduardo-driven) | -- | Nessuno |
...
**Path**: `C:\dev\vault\` (rinominato da `C:\dev\vault-shared\` 2026-05-21)
