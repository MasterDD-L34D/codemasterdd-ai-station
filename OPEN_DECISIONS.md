...
| OD-007 | AA01 capability registry / scan automatico? | ✅ CLOSED 2026-05-28 (DUPLICATE: ARCHON D-007 + `aa01/archon/skills/agent-scanner` gia' built; STRONG-SKIP ratificato via dogfood `FIRST_PRINCIPLES_INFRA_CHECKLIST.md`) | Action AA01-side **SHIPPED 2026-05-28**: drill-down link added to `aa01/AGENTS.md` (AA01 non-git). Validation pending next AA01 task BOOTSTRAP |
...
- **Action AA01-side SHIPPED 2026-05-28 (Eduardo auth esplicita)**: riga aggiunta in cima al DRILL-DOWN table di `aa01/AGENTS.md` -> bold "Tool/agent discovery (BOOTSTRAP mandatorio: ... build on existing, never recreate)" -> link `archon/skills/agent-scanner/SKILL.md` con citazione D-007 + 4 trigger. **Validation pending**: la prossima task AA01 deve invocare lo scanner al BOOTSTRAP senza che Eduardo debba ricordarselo (signal che discovery gap e' chiuso). AA01 e' NON-git -> niente commit, solo edit locale.
