# OPEN_DECISIONS -- Archive (closed OD bodies)

> **Provenance**: extracted from `OPEN_DECISIONS.md` on 2026-06-03 (governance reorg fase 2, HEAD `6a340ac`).
> Contains the full bodies of OD-001..OD-009 (all CLOSED). The live summary table + operating rules
> remain in `OPEN_DECISIONS.md`. Bodies PRESERVED verbatim here, not deleted.
> Note: some bodies contain pre-decision "Eduardo decide" / "Miglior default proposto" text retained
> under a CLOSED verdict -- historical context, the verdict at the top of each OD is authoritative.
...
- **File o moduli coinvolti**: `CLAUDE.md` sezione "Priorità modelli AI" + `docs/reference/patterns/delegation-to-aider.md` + `MODEL_ROUTING.md`.
