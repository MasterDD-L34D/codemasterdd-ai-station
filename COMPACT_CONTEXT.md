...
- **Versione del compact**: v25 (sessione 2026-05-28 closure notte, drift fix v24->v25 cumulative post massive governance cleanup giornata intera). **v24 legacy** = 2026-05-13 stale pre-pivot. **v25 fresh state**:
  - **HEAD origin/main** codemasterdd: `416ee55` (post drift-fix Game-Database CLAUDE.md doc-hygiene anti-pattern #19).
  - **Lessons cumulative**: L-001..L-040 promosse (38+ active in `~/aa01/learnings/`). **3 nuove oggi**: L-038 ESM CLI pathToFileURL guard, L-039 Game branch-protection footgun (path-filtered required-check skipping), L-040 PowerShell native-stderr-under-Stop false-fail.
  - **OPEN_DECISIONS**: **9/9 CLOSED** (OD-001..009 tutti). OD-005 BUILD `FIRST_PRINCIPLES_INFRA_CHECKLIST.md` ~230 righe. OD-007 3-layer agent-scanner cross-fleet deploy (LITE skill global + L3 STRONG-PURE directive + deploy script idempotente sandbox QG). OD-009 ADR-0017 stack DECOMMISSION (LiteLLM+Langfuse+Postgres+dogfood-ui rimosso post-Hybrid-A1).
  - **ADR shipped/changed oggi**: ADR-0017 status **Accepted -> SUPERSEDED-by-ADR-0030**. Cumulative 34 ADR (6 SUPERSEDED visible inline via Status col).
  - **Cross-fleet agent-scanner deploy LIVE** Lenovo (skill `agent-scanner` ora in `~/.claude/skills/` discoverable cross-session, L3 directive in `~/.claude/CLAUDE.md`, 19/19 unit test PASS, sandbox QG mandatory pre-live). Pending Ryzen T13 deploy Eduardo-manual.
  - **Mirror infra completa**: scheduled task `codemasterdd-mirror-backup` Dom 10:00 Ready + `scripts/backup/copy-mirror-to-external.ps1` helper external-drive + runbook.
  - **First-principles audit codemasterdd** dogfood `docs/research/2026-05-28-codemasterdd-first-principles-audit.md`. Cuts eseguiti: 5 sub-agent draft -> `.claude/agents/_dormant/` (21->16 visible), `scripts/README.md` categorize 30+ script, dead-weight `final-research-...v3.md` deleted.
  - **Cross-fleet pull-pass closing**: codemasterdd `416ee55` + Game `31250b5d` (husky restored) + Game-Godot-v2 `efd5bf6` (+107) + Game-Database `13079e2` (+50 + drift fix) + vault `af851b67f` (+15 + gc 99.93% orphan removal) + synesthesia dormant + evo-swarm `10a40ba` (PR #123 weekly merged).
  - **VC governance review** complete: autoresearch DORA/Fowler/Well-Architected verdict STRUTTURA SANA + 4 hardening azionati (CI codemasterdd safety-net + scheduled mirror + Game #2410 fix + privacy guard).
  - **Game #2410 footgun FIXED** via PR #2413 (admin-merged post harsh-reviewer P1 fix) + branch protection swap `[governance, ci-gate]`. Tooling-only PR ora merge senza admin override.
  - **Privacy guard** `.aiderignore` per `docs/ryzen-memory-archive/` (KNOWLEDGE_MAP section 6).
  - **SoT Drift Sentinel** A+B LIVE: Game Action `sot-drift-sentinel` push:main flag `sot-drift-candidate` issue + codemasterdd `sot-drift-verifier` sovereign gated verdict. Prima invocazione reale (vault epigenome §24.6) verdict NO-DRIFT (gia' riconciliato vault `40992953`).
  - **Cognitive protocols cumulative**: P1 refresh-verify + P2 autoresearch + P3 Archon + P4 AA01 + P5 harsh-reviewer + P6 brainstorming + P7 SDMG. 7-protocol framework MATURE.
- **Drift fix tracking v23 (legacy)**: v22 dichiarava HEAD `19d78f9` post PR #62 + 44 PR cumulative; reality 13/5 notte HEAD `51d135a` post PR #74 + 52 PR cumulative 7-13/5 codemasterdd + 2 PR Godot-v2. Cluster 12-13/5 (8 PR cluster post-v22): #63-#68 12/5 sera (Bundle 1+2+3 + residual + vault handoff + re-eval calendarizzati) + #70 closure notte 12/5 + #71+#72+#73+#74 13/5 sera+notte (cross-PC R-items Phase 1 + Q1/Q2/Q3 ADR-0027 Phase 2 + closure JOURNAL/STATUS/L-014). Cross-PC ecosystem documented 13/5 notte: SSH key-based auth Lenovo->Ryzen ed25519 + 9 drift CLAUDE.md fixati + H7 ANTHROPIC tier-0 setup + ACL keys.env hardened + DHCP reservation TIM HUB DGA4132 AGTHP drift class IPs PERMANENTLY KILLED (Lenovo `.10` + Ryzen `.11` reservation outside DHCP pool) + vault llm-routing IP fix `.121->.10` + Aider whitelist Ryzen propagation. **6gg residui pre-Max** (19/05).
- **Data ultimo aggiornamento**: 2026-05-28 closure notte (post massive governance cleanup + 3-layer cross-fleet agent-scanner deploy + ADR-0017 decommission + first-principles audit dogfood + closing pull-pass + drift-fix Game-Database).

## Stato attuale
- **Barra globale ~99.97%**: Fase 6 + Fase 7 CLOSED. ADR attivi 0015/0017/0018-0022 + 0024 + 0025 + 0026 + **0027 cross-PC clone architecture (Accepted early 13/5 notte)** Accepted | 0009/0016/0023 Proposed. Hyperspace ABANDONED definitivo. **6gg residui pre-Max al 13/5 notte**. **Cross-PC ecosystem MAJOR doc 13/5**: 9 drift CLAUDE.md fixati (IP×2 + Vgit username + AI stack + Game/Synesthesia Ryzen path + Vault origin Ryzen reframe + ACL claim + ANTHROPIC provider) + ADR-0027 (Q1/Q2/Q3/R1/R3 RESOLVED empirical) + DHCP reservation drift class killed permanent + L-2026-05-014 autoresearch first pattern promoted. Sessione 13/5 notte raggiunta stato **METHODOLOGY FRAMEWORK MATURE + CROSS-PC ECOSYSTEM REALITY DOCUMENTED + DRIFT CLASS PERMANENTLY KILLED**.
- HEAD origin/main `51d135a` (post **merge PR #74 13/5 notte closure JOURNAL+STATUS+L-014**). PR #63-#74 cluster 12-13/5 sera+notte: 8 PR (Bundle 1+2+3+residual+vault+re-eval #63-#68 12/5 sera + closure JOURNAL #70 12/5 notte + Phase 1 R2/R4+R5 #71+#72 13/5 sera+notte + Phase 2 ADR-0027 Q1/Q2/Q3/R1/R3 #73 + closure JOURNAL/STATUS/L-014 #74). Cumulative 7-13/5: **52 PR mergeati codemasterdd** + **2 PR Godot-v2** codemasterdd-authored. delete_branch_on_merge auto-toggle attivo. **Worktree corrente `claude/compact-v21-sprint02-preflight-2026-05-13`** (sessione 13/5 notte ULTRA-finale auto-mode).
- **AA01 cleanup state 13/5 notte**: workspace 0 task attivi, archive **15 entries**, **14 lessons cumulative L-001..L-014 in `learnings/`**:
  - `L-2026-04-001` (process audit-replay pattern sandbox-restricted-agent)
  - `L-2026-05-002-hyperspace-audit-cycle.md` (3 anti-pattern + 4 pattern positive)
  - `L-2026-05-003-cross-repo-pattern-adoption.md` (cross-check governance interna)
  - `L-2026-05-004-aa01-conditional-fit-meta-assessment.md`
  - `L-2026-05-005-dogfood-driven-self-bug-discovery.md` (governance-lint MVP)
  - `L-2026-05-006-karpathy-autoresearch-archon-calibrate-methodology.md`
  - `L-2026-05-007-gh-api-empirical-stars-mandatory.md`
  - `L-2026-05-008-claude-code-plugin-install-archon-falsifying-experiment.md`
  - `L-2026-05-009-archon-defer-pivot-pattern.md`
  - `L-2026-05-010-reflexive-methodology-audit-pattern.md` (apply method to method 7-step)
  - `L-2026-05-011-applicative-optimization-audit-pattern.md` (empirical smoke FAIL+PASS coppie)
  - `L-2026-05-012-vault-sibling-peer-write-under-explicit-authorization.md` (boundary nuance 3-step)
  - `L-2026-05-013-re-eval-calendarizzati-pattern.md` (deadline-driven -> trigger-emergent shift 4-step)
  - `L-2026-05-014-autoresearch-first-pattern.md` (5-step pattern technical problems + forum technical Italian inclusion, TIM AGTHP DHCP case study)
- **Vault sibling-peer state 13/5 notte**: **7/7 PRODUCTION + llm-routing IP fix synced** 3-way Ryzen/Lenovo/origin (HEAD `1abaa743` "fix(routing): lenovo ollama ip 121 -> 10 post dhcp reservation"). Origin lineage reframe (Ryzen=source, Lenovo=clone downstream) documentato CLAUDE.md + memory.
- **Coda PR codemasterdd**: VUOTA post-cluster 13/5 notte ULTRA-finale.
...
1. **Next session focus** (integration roadmap, vedi `docs/plans/integration-aa01-vault-hyperspace-2026-05.md`):
   - **Obiettivo 1 (45min)**: AA01 inbox capture `aa01-003-vault-integration-readonly` + `aa01-004-hyperspace-phase-1-privacy-audit` + classify + promote 1 (Vault primo)
   - **Obiettivo 2 Phase 0 (45min)**: Vault read-only deep dive parziale (`C:/dev/vault-shared/`, Quality Gate workflow + LLM routing matrix + 7 production agents)
   - **Validation H12 hook fix**: `ls .claude/.session-start-head` prima cosa -- se marker esiste -> fix v18 confermato, se assente -> fallback `git rev-parse --show-toplevel`
2. **Eduardo direct (residuo standalone unico)**:
   - **H7 setup ANTHROPIC_API_KEY** in `~/.config/api-keys/keys.env` (~5min via Anthropic Console) -- pre-19/05 priority
3. **Calendarizzati passive** (no immediate action):
   - 2026-05-19 Claude Max expiration (**8gg residui al 11/5 sera**, stack pronto)
...
3. **`docs/plans/integration-aa01-vault-hyperspace-2026-05.md`** -- next session focus (NEW)
4. `AGENTS.md` SE sessione e' Codex/OpenCode/sandbox-based -- preamble anti-confusion
5. `STATUS_MULTI_REPO.md` -- dashboard cross-repo (Game Sprint Impronta, Dafne Atto 2)
6. `BACKLOG.md` + `OPEN_DECISIONS.md` -- cosa e' aperto
7. `SPRINT_02.md` -- sprint corrente Fase 8 sovereign (T2/T5/T7 restanti)
8. ADR rilevanti se task tocca topic noto

**Prima azione next session**: `ls .claude/.session-start-head` -- validation hook fix H12 v18. Se marker presente -> SessionStart hook funzionante, Stop hook attivo. Se assente -> debug fallback (hardcode path via `git rev-parse --show-toplevel`).
