...
- **Wrapper CLI per delegazione** (in `C:\Users\edusc\.local\bin\`, eseguibili da cmd.exe):
  ...
  - **Aider cloud (tier 3-4, aggiunti 2026-04-23 combo F+D, vedi ADR-0013)**:
    - `aider-groq <file>` → groq/llama-3.3-70b-versatile + diff + no-auto-commits — **DEPRECATED 2026-05-13** post T1 #29+#35: LiteLLM-Groq adapter bug streaming hang (Issue #9296+#12660+#4804+#16040). Use `aider-groq-bypass` invece
    - `aider-groq-bypass <file>` → openai/llama-3.3-70b-versatile via `--openai-api-base https://api.groq.com/openai/v1` — **VIABLE 2026-05-13 T1 #36 PASS**: bypass LiteLLM Groq adapter via OpenAI-compatible endpoint, ~30s latency vs 5min stuck (autoresearch L-2026-05-014 pattern). Groq TPM 300K (era 12K free tier, ora 300K post-Tier1). $0 free tier
