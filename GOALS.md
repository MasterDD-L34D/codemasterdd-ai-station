...
| Game-Database | **RFC #4 traits DONE end-to-end** (06-14): loop DB->Game live (glossary Game#2750 + canon #2752 + reference #2755 + mirror #2758; GATE G1->G6 verde, 10 cicli, fidelity-gated). **Species export ACTIVE (06-17)**: scope ratificato (RFC #209: S-Q1 sourceExtras parity / S-Q2 description non-exported / S-Q3 per-file+index, canonical-index downstream) -> **Sp1a SHIPPED #214** (provenance + snapshot determinism, recompute-at-snapshot) -> **Sp1b DISPATCHED** (Jules issue #215) | (PARKED) biome/eco export: YAML (js-yaml + order-preserve + eco model-gap extend-vs-sourceExtras), scope-doc+ratifica prima del dispatch; audit-UI hardening | Robust versioned auditable content backend (evo:import) | feeds Game |
...
- **Game-Database**: ~~versioned reads~~ **DONE** (#180) -> ~~RFC #4 scoping + S1 trait shadow-exporter~~ **DONE** (RFC ratified #182/#183; traits loop DB->Game live 06-14). **Active (06-17)**: species export -- scope ratificato #209, **Sp1a SHIPPED #214** (provenance + snapshot determinism), **Sp1b DISPATCHED** (exporter, issue #215). biome/eco still parked. Eduardo-sovereign gate.
