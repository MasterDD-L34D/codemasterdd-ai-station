...
from flask import Flask, render_template, request, redirect, url_for, jsonify, flash, Response, Blueprint, current_app, session, abort
...
@main_bp.route("/")
def index():
    entries = current_app.db.list_entries(limit=50)
    stats = aggregate_stats(current_app.db.list_entries())
...
def list_entries():
    entries = current_app.db.list_entries(limit=500)
    return render_template("entries.html", entries=entries)

@main_bp.route("/entries/export.csv", methods=["GET"])
def export_entries_csv():
    entries = current_app.db.list_entries(limit=10000)
...
def new_entry():
    if request.method == "POST":
        ...
        pulled = enrich_from_langfuse(payload, current_app.lf)
        entry_id = current_app.db.insert_entry(payload)
        if pulled:
            flash(f"Entry #{entry_id} saved. Auto-filled from Langfuse: {', '.join(pulled)}.", "success")
        else:
            flash(f"Entry #{entry_id} saved.", "success")
        return redirect(url_for("main.list_entries"))
...
def delete_entry(entry_id: int):
    ...
    current_app.db.delete_entry(entry_id)
    flash(f"Entry #{entry_id} deleted.", "success")
    return redirect(url_for("main.list_entries"))

@main_bp.route("/stats")
def stats_view():
    entries = current_app.db.list_entries()
...
@main_bp.route("/dafne")
def dafne_view():
    snapshot = current_app.dafne.full_snapshot()
...
@main_bp.route("/api/entries", methods=["GET", "POST"])
def api_entries():
    if request.method == "POST":
    ...
    entries = current_app.db.list_entries(limit=int(request.args.get("limit", 100)))
    return jsonify([entry_to_dict(e) for e in entries])

@main_bp.route("/api/stats")
def api_stats():
    return jsonify(aggregate_stats(current_app.db.list_entries()))
...
@main_bp.route("/api/dafne/snapshot")
def api_dafne_snapshot():
    """Proxy snapshot aggregato da Dafne :5000."""
    return jsonify(current_app.dafne.full_snapshot())
