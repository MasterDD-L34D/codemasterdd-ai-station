...
**Ultimo refresh**: **2026-05-22 pomeriggio** (repo-health-auditor Ryzen session: evo-swarm Option A PR #116 open + branch cleanup 61 remote branches, Game feat/m1-sistema-memory 3-ahead, Godot CAMP-3c 2-ahead, vault 4-ahead 2 open PRs, Game-DB synced, synesthesia dormant invariato).

**Codemasterdd state 2026-05-22**: HEAD main `59ccb98` (session handoff + goals refresh + spec rejection autonomous-next-point + D2b design + agent update). Clean working tree, synced 0/0. 0 open PRs. Session on Ryzen (DESKTOP-T77TMKT / VGit).

**Cross-repo state 2026-05-22**: evo-swarm `4c37100` + PR #116 OPEN (Option A born-ready, Eduardo merge pending). Game `04f78342` feat/m1-sistema-memory-debrief-surface 3-ahead-main (no open PRs). Godot-v2 `3fcdc7f` CAMP-3c handoff branch 2-ahead-main (no open PRs). Game-DB `223fe05` synced clean. vault `dcc7a167` 4-ahead-upstream + 2 open PRs (#163 OD-056, #164 backstop DRAFT). synesthesia `05f8a92` dormant (invariato aprile). Stato verificato -> §Ecosystem-audit 2026-05-22 sotto.
...
|------|--------|-------------|------------------|---------|
| **codemasterdd-ai-station** | HEAD main `59ccb98` (2026-05-21). Clean, synced, 0 open PRs. Session: session handoff + goals refresh + spec rejections (autonomous-next-point) + D2b design. **Governance + policy hub status: NOMINAL**. Claude Max ri-acquistato +1mo ~17/06/2026. ADR-0030 Hybrid A1 Accepted. | Monitor cross-repo stato + agenda Eduardo | ~2026-06-17 Claude Max expiry | Nessuno |
| **Synesthesia** | Dormant, HEAD `05f8a92` (invariato da aprile 2026, last commit 2026-04-16) | Riattiva pre-esame UniUPO | ~agosto 2026 | Nessuno (dormant intenzionale) |
| **Game (Evo-Tactics Vue3)** | HEAD main `9a8e71dc` (2026-05-27), synced, 0 open PRs Game-side. **Fase-1 Spore Moderate genetic model = CLOSURE COMPLETE** (DoD §6 full): 6 PR runtime merged 2026-05-26 (#2393 plan v3 + #2394 RECON-01 baseline 20/20 + #2395 RECON-04a ripple-audit/cost-charging-guard + #2396 RECON-02 derived_ability x12 + #2397 RECON-03a bingo rebalance tank_plus 15.9% + #2398 RECON-04b complexity-budget G2). RECON-06 closure cross-repo MERGED 2026-05-27 (vault #200 SoT §24.3/§24.6 D-HEIR canonical + Godot-v2 #354 PRD overlay). **Fase-2 DONE**: #2399 cross-lineage isolation + #2400 hybrid fusion engine + Godot-v2 #356 genetics_api unify (offspring-ritual proof-of-life). **Fase-3 epigenome params RATIFIED** #2401 (Lamarck-lite: inheritance_weight 0.3 / decay 0.6 / regression 0.3 / cap +/-0.2; discrete-expression coherent vs Niche-standard + P2 "not-continuous-sim"; research doc + coherence-check vs publisher comparables). Finding: `mating_trigger.gd` = client PREVIEW (NOT canonical divergence; commit already backend) -> full-migration non-blocker. RECON-03b deferred Fase-1.5. | Fase-3 epigenome BUILD (engine net-new, lock playtest N>=40) = next major, plan-first | Eduardo-driven | Nessuno |
| **Game-Godot-v2** | Branch `claude/handoff-2026-05-22`, HEAD `3fcdc7f6` (2026-05-22). **2 commits ahead origin/main** (local handoff doc + CAMP-3c TV loop re-entry design). No open PRs. Active: CAMP-3b TV DebriefView mount (#348) merged main; CAMP-3c design doc authored. Governance interna autosufficiente. Daily-ship. | CAMP-3c implementation planning (Eduardo-driven) | No fixed | 2 ahead non-PR'd (handoff doc + design, probably intentional staging) |
| **Dafne swarm (evo-swarm)** | HEAD main `4c37100` (2026-05-22), clean, synced 0/0. **PR #116 OPEN** (`dafne/option-a-born-ready`): born-ready artifact enrichment hot-path Option A -- **AWAITING Eduardo merge**. Branch cleanup done: 61 merged remote branches deleted, remote now main-only + `dafne/option-a-born-ready`. 13 local branches present Ryzen-side (feature staging, non-pushed). Last remote commit: fix(verify) load species from data/core/species/ catalog (#115). | **Eduardo merge PR #116** (Option A born-ready) | No fixed | PR #116 open, Eduardo decision required |
...
| **vault-shared (Vault Knowledge Mgmt)** | Ryzen clone (`C:/dev/vault`) HEAD `dcc7a167` (2026-05-21). **4 commits ahead upstream** (not yet pushed). **2 open PRs**: #163 `claude/od-056-ratify-l2scale-2026-05-22` OPEN (OD-056 ratify + gitignore lint-reports), #164 `claude/coherence-backstop-2026-05-22` DRAFT. Untracked `.claude/` dir locally. vault-shared (Lenovo path `C:/dev/vault-shared`) does NOT exist on Ryzen (Lenovo-only clone). Privacy: sovereign-only. | Eduardo merge/review PR #163 (OD-056), triage #164 backstop, push 4-ahead commits | No fixed | 4 commits not pushed; 2 PRs Eduardo-decision |
| **Game-Database** (Ryzen `C:\dev\Game-Database`) | HEAD main `223fe05` (2026-05-22), clean, synced 0/0. No open PRs. Active: Phase B2 schema versioning (soft-delete + restore masters, PRs #158-#162). Taxonomy CMS upstream content provider. | Monitor (Eduardo-driven) | No fixed | Nessuno |
